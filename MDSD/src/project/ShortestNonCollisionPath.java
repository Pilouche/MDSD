			...
			for (Boundary b : environment.getBoundries()) {
				...
				if (b.getP1x() == b.getP2x()) {
					...
					if (zValue > one.getZ())
						...
						cutOnRise = 0.5;
					if ((zValue <= b.getP1z() && zValue >= b.getP1z())
							|| (zValue >= b.getP1z() && zValue <= b.getP1z())) {
						if (calculatePathBetweenPoints(environment,
								new Position(b.getP1x() + oneBigTwo, b.getP1z() + cutOnRise), two)
										.getLength() < calculatePathBetweenPoints(environment,
												new Position(b.getP2x() - oneBigTwo, b.getP2z() + cutOnRise), two)
														.getLength()) {
							Set<Position> p = new HashSet<Position>();
							Position tempPos = new Position(b.getP1x() + oneBigTwo, b.getP1z() - cutOnRise);
							p.add(tempPos);
							PathAndLength temp = calculatePathBetweenPoints(environment,
									new Position(b.getP1x() + oneBigTwo, b.getP1z() + cutOnRise), two);
